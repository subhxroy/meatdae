...
<p class='exzoom_preview'>
   ...
   */
  /**
   * UPDATED function to ensure all images are loaded before init
   * @param images
   */
  function checkLoadedAllImages(images) {
    ...
    images.each(function () {
      // Use the 'load' event to be certain the image is ready
      $(this).on('load', function() {
        loaded_images_counter++;
        if (loaded_images_counter === all_images_num) {
          // Only initialize after all images have confirmed loading
          init();
        }
      }).on('error', function() {
        // Handle cases where an image might fail to load
        console.error("Image failed to load: " + this.src);
        all_images_num--; // Decrement the total count so we don't wait for a broken image
        if (loaded_images_counter === all_images_num) {
          init();
        }
      });

      // Also check the 'complete' property for images that might be cached
      if (this.complete) {
        $(this).trigger('load');
      }
    });
