...
(function () {
    ...
    onAuthStateChanged(auth, user => {
        ...
        onSnapshot(doc(db, "orders", orderId), (docSnap) => {
            if (!docSnap.exists()) {
                ...
                if (el) el.innerText = "Order Not Found";
                return;
            }
            ...
            if (!customerLat || !customerLng) {
                ...
                if (loc) {
                    ...
                    if (customerLat && customerLng) initTrackMap(customerLat, customerLng);
                }
            }
            ...
            if (order.status === 'OUT_FOR_DELIVERY') {
                ...
                if (callBtn && order.riderPhone) callBtn.href = `tel:${order.riderPhone}`;
                ...
                if (order.riderId && customerLat && customerLng && order.riderId !== currentRiderId) {
                    ...
                    console.log("[TRACKING] Starting listener for:", order.riderId);
    ...
    function updateStatusUI(order) {
        ...
        const stagesInfo = {
            ...
            'PREPARING': {
                badge: 'Processing', badgeColor: '#feebe0', badgeText: 'var(--primary)',
                title: 'Preparing Your Meat',
                desc: 'Butcher is preparing your premium cuts',
        ...
        stageIds.forEach(id => {
            ...
            // If the order is cancelled, we don't show any active or done dots
        ...
        // Prominent Cancellation Notice
        if (order.status === 'CANCELLED') {
            const container = document.getElementById('progress-tracker');
            if (container && !document.getElementById('cancellation-overlay')) {
                const overlay = document.createElement('div');
                overlay.id = 'cancellation-overlay';
                overlay.innerHTML = `
                    <div style="background: rgba(220, 53, 69, 0.1); border: 2px dashed #dc3545; border-radius: 16px; padding: 25px; text-align: center; margin-bottom: 20px;">
                        <i class="fas fa-ban fa-3x mb-3" style="color: #dc3545;"></i>
                        <h4 style="color: #dc3545; font-weight: 800; font-family: 'Barlow', sans-serif;">ORDER CANCELLED</h4>
                        <p class="mb-0 text-muted">This order has been cancelled by the store.</p>
                        <a href="index.html" class="btn btn-danger mt-3 rounded-pill px-4 fw-bold">Order Something Else</a>
                    </div>
                `;
                container.prepend(overlay);
                // Hide the timeline line
                const line = document.getElementById('progress-fill');
                if (line) line.style.display = 'none';
                const bgLine = document.querySelector('.progress-line');
